//
//  euler8.swift
//  Project Euler
//
//  Created by Ron Ramirez on 11/6/16.
//  Copyright © 2016 Mochi Apps. All rights reserved.
//

import Foundation

func euler8() {
    print("Largest Product in Series")
    let numberString = ""
    
    _ = "7316717653"
    
    func largestProduct(length : Int, data : String){
        var index = 0
        var max = 0
        var chList = [Character]()
        for Character in data.characters{
            chList.append(Character)
        }
        
        //condition stop last character to check before end of 1000-digit
        let condition = chList.count - length + 1
        print(condition)
        
        
        //While index does not equal length
        while(index != condition){
            //Print current index and matching character
            //print(index, chList[index])
            var product = 1
            for x in index ..< (index + length){
                print(chList[x], terminator:"")
                product *= Int(String(chList[x]))!
            }
            
            //Assign product to max if its bigger
            if product > max{
                max = product
            }
            
            print(" = \(product)")
            //Increase Index
            index += 1
        }
        print("The length of number you want to test is \(length)")
        print("The greatest product is \(max)")
        print("The length of the number is \(chList.count)")
    }
    

    largestProduct(length : 13, data: numberString)
    //print(numberString)

}

